package com.credomatic.grpod.ecommerce.android.gptt.testingtool.utilities.security;
...
public class Md5 {
    ...
    private final long lngState[] = new long[4];
    ...
    // Static Methods
    //
    private static long[] decode(final char bytBlock[]) {
    ...
    private static void transform(final long lngState[], final char bytBlock[]) {
    ...
    private void update(final char bytInput[], final long lngLen) {
        int index = (int) (this.lngByteCount % 64), i;
        this.lngByteCount += lngLen;
        int partLen = 64 - index;

        if (lngLen >= partLen) {
            System.arraycopy(bytInput, 0, this.bytBuffer, index, partLen);
            transform(this.lngState, this.bytBuffer);

            for (i = partLen; i + 63 < lngLen; i += 64) {
                System.arraycopy(bytInput, i, this.bytBuffer, 0, 64);
                transform(this.lngState, this.bytBuffer);
            }
            index = 0;
        } else {
        ...
        System.arraycopy(bytInput, i, this.bytBuffer, index, (int) (lngLen - i));
    }

    private void md5final() {

        int index, padLen;
        final char bytBits[] = new char[8];
        final long bits = this.lngByteCount * 8;
    ...
    private StringBuffer toHexString() {
        ...
        for (int j = 0; j < 4; ++j) {
            for (int i = 0; i < 32; i += 8) {
                ...
                if (myByte < 16) {
                    myString.append("0").append(Long.toHexString(myByte));
                } else {
    ...
    public String getMd5(final String hashParams) {
        final char chrTestData[] = hashParams.toCharArray();
        this.update(chrTestData, chrTestData.length);
        this.md5final();
        return this.toHexString().toString();
    }
}
